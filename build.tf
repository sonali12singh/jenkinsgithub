...
resource "aws_security_group" "admin" {
  ...
  vpc_id      = data.aws_vpc.existing.id
  # ICMP (ping) only from inside VPC for diagnostics
  ingress {
    from_port   = -1
    to_port     = -1
    protocol    = "icmp"
...
resource "aws_security_group" "agent" {
  ...
  # Allow communication from Admin node private IPs/Security Group (more secure)
  ingress {
    from_port   = 0
    # to_port     = 65535
    # protocol    = "tcp"
...
resource "aws_security_group" "chrome" {
  ...
  # Allow all TCP from admin SG (for management or special comms)
  ingress {
    from_port   = 0
    to_port     = 65535
    protocol    = "tcp"
