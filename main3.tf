...
locals {
  ...
  sl_subnet_bit_size   = 32 - tonumber(split("/", data.aws_subnet.standalone.cidr_block)[1])
...
data "aws_route_table" "standalone" {
  filter {
    name   = "route-table-id"
...
#------- it create a deplicate route if one already exits  in the route table then we can skip 
# resource "aws_route" "default_gw" {
#   route_table_id         = data.aws_route_table.standalone.id
#   destination_cidr_block = "0.0.0.0/0"
#   gateway_id             = data.aws_internet_gateway.existing.id
# }
#-----  it might be redundant if subnet is already associated  with the same route table 
...
resource "aws_security_group" "agent" {
  ...
  ingress {
    ...
    # cidr_blocks = [aws_instance.admin.private_ip] # security_groups = [aws_security_group.admin.id]
    # cidr_blocks = [data.aws_subnet.standalone.cidr_block]
...
resource "aws_db_subnet_group" "default" {
  ...
  # subnet_ids = [data.aws_subnet.standalone.id, data.aws_subnet.backup[count.index]]
  subnet_ids = [
    data.aws_subnet.standalone.id,
    data.aws_subnet.backup.id
  ]
  tags = {
    Name = "${var.vpc_name}-rds"
    env  = var.vpc_name
